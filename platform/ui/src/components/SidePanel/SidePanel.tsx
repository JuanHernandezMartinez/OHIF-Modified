...
const SidePanel = ({ side, className, activeTabIndex: activeTabIndexProp, tabs, onOpen }) => {
  ...
  //Con esto se controla el abrir o cerrar el panel de la derecha por defecto, cambio el MobilVewi a true y ya funciona siempre abierto
  ...
  const getCloseIcon = () => {
    return (
      <div
        className={classnames(
          'flex h-[10px] cursor-pointer items-center justify-center',
            ...
            return (
              <React.Fragment key={tabIndex}>
                {tabIndex % numCols !== 0 && (
                  <div
                    className={classnames(
                      'flex h-[10px] w-[2px] items-center bg-black',
                      tabSpacerWidth
                    )}
                  >
